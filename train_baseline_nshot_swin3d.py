...
class GlobalColorTransform(nn.Module):
    ...
    def forward(self, x):
        ...
        x_trans = x_trans + self.bias.view(1, C, 1, 1, 1) + mean
        return x_trans.clamp(0, 1)
...
class TransformModule(nn.Module):
    ...
    def forward(self, x):
        ...
        # Commented out in baseline - keeping residual structure for consistency
        #x_orig = x
...
def main_exp(args):
    ...
    model = DualHeadSwin3D(0, num_classes_R).to(device)
    ...
    for epoch in range(epochs):
        loss = train_real( model, dataloader_R, optimizer, scheduler,
            device, lambda_mmd=args.lambda_mmd, viz_interval=100, epoch=epoch)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train baseline n-shot Swin3D model')
